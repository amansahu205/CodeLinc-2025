...
interface PlanComparisonModalProps {
  ...
  onSelectPlan: (planId: number, planName: string, planCost: number) => void
}
...
export function PlanComparisonModal({ isOpen, onClose, benefit, selectedPlanId, onSelectPlan }: PlanComparisonModalProps) {
  const benefitPlans = benefit?.plans || plans
  useEffect(() => {
            ...
            <div className="backdrop-blur-md bg-white/80 rounded-xl sm:rounded-2xl shadow-xl max-w-5xl w-full max-h-[90vh] overflow-hidden border border-white/40">
              ...
              <div className="bg-gradient-to-r from-[#a50034] to-[#7d1f3c] p-4 sm:p-6 flex items-center justify-between">
                <h2 id="modal-title" className="text-lg sm:text-2xl font-bold text-white">
                  Compare {benefit?.name || 'Benefit'} Plans
                </h2>
                <button
                  onClick={onClose}
                      ...
                      <motion.div
                        ...
                        className={`relative rounded-xl p-4 sm:p-6 space-y-3 sm:space-y-4 transition-all duration-300 ${
                          plan.id === selectedPlanId
                            ? "bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-600 shadow-lg"
                            : "bg-[#F7F6F5] border border-[#DADADA] hover:shadow-md"
                        }`}
                      >
                        {plan.id === selectedPlanId ? (
                          <Badge className="absolute -top-2 sm:-top-3 left-1/2 -translate-x-1/2 bg-green-600 text-white border-none shadow-md text-xs">
                            ✓ Selected
                          </Badge>
                        ) : plan.recommended && (
                        ...
                        <div className="flex items-center gap-2 sm:gap-3 pt-2">
                          ...
                          <div>
                            <h3 className="text-lg sm:text-xl font-bold text-[#333333]">{plan.name || plan.fullName}</h3>
                            {plan.fullName && plan.name !== plan.fullName && (
                        ...
                        <div className="space-y-2 sm:space-y-3">
                          <div className="flex justify-between items-center py-2 border-b border-[#DADADA]">
                            ...
                            <span className="font-semibold text-[#333333] text-xs sm:text-base">
                              ${typeof plan.cost === 'number' ? plan.cost.toFixed(2) : plan.premium}/paycheck
                            </span>
                          </div>
                        ...
                        <Button
                          onClick={() => onSelectPlan(plan.id, plan.name || plan.fullName, parseFloat(plan.cost || plan.premiumValue))}
                          className="w-full transition-all duration-300 text-xs sm:text-sm h-9 sm:h-10 bg-gradient-to-r from-[#a50034] to-[#7d1f3c] hover:from-[#7d1f3c] hover:to-[#a50034] text-white"
                        >
