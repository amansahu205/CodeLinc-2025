...
export function ChatSidebarPlaceholder({
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      console.log('[CHAT] Sending message to backend:', userQuery)
      ...
      const res = await fetch('http://benefits-env.eba-vdatx5t9.us-east-1.elasticbeanstalk.com/api/chat', {
        method: 'POST',
        headers: {
      ...
      console.log('[CHAT] Backend response:', result)
      
      // Update recommendations if provided
      if (result.recommendations && result.recommendations.length > 0) {
        console.log('[CHAT] Updating recommendations:', result.recommendations)
      ...
      const aiResponse: Message = {
        ...
        type: "assistant",
        content: result.reply || 'No response from AI',
        timestamp: new Date(),
      }
      ...
      const aiResponse: Message = {
        id: (Date.now() + 1).toString(),
        type: "assistant",
        content: getMockResponse(userQuery),
        timestamp: new Date(),
      }
      setMessages((prev) => [...prev, aiResponse])
    } finally {
  ...
  const handleQuickAction = async (action: string) => {
    ...
    try {
      const token = localStorage.getItem('auth_token')
      console.log('[CHAT] Quick action:', action)
      ...
      console.log('[CHAT] Quick action response:', res.status)
      ...
      const result = await res.json()
      console.log('[CHAT] Quick action result:', result)
      ...
      const aiResponse: Message = {
        id: (Date.now() + 1).toString(),
        type: "assistant",
        content: getMockResponse(action),
        timestamp: new Date(),
      }
      setMessages((prev) => [...prev, aiResponse])
    } finally {
